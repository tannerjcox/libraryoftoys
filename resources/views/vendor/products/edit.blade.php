...
@section('content')
    @if(isset($product))
        {!! BootForm::open()->put()->action(route('vendor-products.update', $product->id)) !!}
        {!! BootForm::bind($product) !!}
        {!! BootForm::hidden('id') !!}
    @else
        {!! BootForm::open()->post()->action(route('vendor-products.store')) !!}
    @endif
    <div class="panel panel-default">
        <div class="panel-heading">
            @if(isset($product))
                <span class="pull-right">{!! link_to_route('vendor-product.show', 'View Product', ['name' => str_slug($product->name), 'id' => $product->id, 'preview' => true], ['target' => '_blank', 'class' => 'btn btn-info']) !!}</span>
            @endif
            <h4>
                @if(isset($product))
